...
openssl req -new -key server-key.pem -subj "/CN=system:node:${service}.${namespace}.svc /OU=system:nodes /O=system:nodes" -out ${tmpdir}/server.csr -config ${tmpdir}/csr.conf
...
kubectl delete ValidatingWebhookConfiguration validation-webhook 2>/dev/null || true
